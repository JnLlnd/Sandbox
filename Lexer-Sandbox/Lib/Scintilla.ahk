; from Adventure-3.0.4 aka AutoGui (https://www.autohotkey.com/boards/viewtopic.php?f=64&t=89901)
Class Scintilla {
    hWnd            := 0        ; Component handle.
    Notify          := ""       ; Name of the function that will handle the window messages sent by the control.
    IdFrom          := 0        ; Identifier of the control sending the message.
    ScnCode         := 0        ; The SCN_* notification code.

    ; Field                     ; Messages which set these fields.
    Position        := 0        ; SCN_STYLENEEDED, SCN_DOUBLECLICK, SCN_MODIFIED
    ...
    Margin          := 0        ; SCN_MARGINCLICK, SCN_MARGINRIGHTCLICK
    ...
    __Call(msg, ByRef wParam := 0, ByRef lParam := 0, Params*) {
        ...
        Return __sendEditor(this.hWnd, msg, wParam, lParam)
    }
}

/*
    Function: LoadSciLexer
    This function initializes the Scintilla component.

    Parameters:
    LoadSciLexer([DllPath, Notify])

    DllPath     -   Path to SciLexer.dll. If omitted, the function looks for it in A_ScriptDir.
    Notify      -   Notification handler.
*/

LoadSciLexer(DllPath := "Scintilla.dll", Notify := "__SciNotify") {
...
/*
    ...
    Parameters:
    __Add(hParent, [x, y, w, h, Style, ExStyle])

    hParent     -   hWnd of the parent control
    x           -   x position for the control
    y           -   y position for the control
    w           -   Width of the control
    h           -   Height of the control
    Style       -   Control style. Default: WS_CHILD | WS_VISIBLE | WS_TABSTOP.
    ExStyle     -   Extended control style. Default: WS_EX_CLIENTEDGE (border).

    Returns:
    HWND        -   Component handle.
*/

__Add(hParent, x := 5, y := 5, w := 590, h := 390, Style := 0x50010000, ExStyle := 0x200) {
    Local hSci := DllCall("CreateWindowEx"
                 , "UInt", ExStyle
                 , "Str" , "Scintilla"
                 , "Str" , ""
                 , "UInt", Style
...
/*
    ...
    This is an internal function and it is not needed in normal situations.
    Please use the Scintilla object to call all functions. They call this function automatically.

    Parameters:
    __SendEditor(hWnd, msg, [wParam, lParam])

    hWnd    -   The hWnd of the control that you want to operate on.
    msg     -   The message to be posted. See <http://www.scintilla.org/ScintillaDoc.html> for details.
    ...
    Example:
    __SendEditor(hSci1, "SCI_SETMARGINWIDTHN", 0, 40) ; Set the margin 1 to 40px on the first component.
*/
...
__SendEditor(hWnd, msg := 0, wParam := 0, lParam := 0) {
    ...
    Return DllCall(%hWnd%_df, "Ptr", %hWnd%_dp, "UInt", msg, "Ptr", wParam, "Ptr", lParam)
}

/*
    Function : __SciNotify
    This is the default function that will be called when the WM_NOTIFY message has been received.
    The message is tracked as soon as you add a new Scintilla component.
    ...
    Returns:
    This function sets some variables on the SciObject to which the component belongs to
    and proceedes to call your user defined notify function which can be set in SciObj.Notify.
...
__SciNotify(wParam, lParam, msg, hWnd) {
    ...
    __SciObj                 := __SCI(NumGet(lParam + 0, 0, "Ptr")) ; Returns original object
    __SciObj.IdFrom          := NumGet(lParam + A_Ptrsize * 1)
    __SciObj.SCNCode         := NumGet(lParam + A_Ptrsize * 2)

    __SciObj.Position        := NumGet(lParam + OffPosition)
    __SciObj.Ch              := NumGet(lParam + OffCh)
    __SciObj.Modifiers       := NumGet(lParam + OffModifiers)
    __SciObj.ModType         := NumGet(lParam + OffModType)
    __SciObj.Text            := NumGet(lParam + OffText)
    __SciObj.Length          := NumGet(lParam + OffLength)
    __SciObj.LinesAdded      := NumGet(lParam + OffLinesAdded)
    ;__SciObj.macMessage      := NumGet(lParam + (x64 ? 56 : 40)) ; SCN_MACRORECORD
    ;__SciObj.macwParam       := NumGet(lParam + (x64 ? 64 : 44))
    ;__SciObj.maclParam       := NumGet(lParam + (x64 ? 72 : 48))
    __SciObj.Line            := NumGet(lParam + OffLine)
    ;__SciObj.foldLevelNow    := NumGet(lParam + (x64 ? 84 : 56))
    ;__SciObj.foldLevelPrev   := NumGet(lParam + (x64 ? 88 : 60))
    __SciObj.Margin          := NumGet(lParam + OffMargin)
    __SciObj.ListType        := NumGet(lParam + OffListType)
    __SciObj.x               := NumGet(lParam + OffX)
    __SciObj.y               := NumGet(lParam + OffY)
