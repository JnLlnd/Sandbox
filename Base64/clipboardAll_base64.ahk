...
; just to confirm that the decoding succeded
oFile := FileOpen(A_ScriptDir . "\Clipboard.png", "w")
oFile.RawWrite(oBin, intBytes)
oFile.Close()
BinImgToBITMAP(intBytes, HBITMAP, oBin) ; not working
...
; gosub, Base64ToClipboard
gosub, Base64ClipboardAll
; gosub, Base64Image
...
; voir https://www.autohotkey.com/boards/viewtopic.php?p=362293#p362293
; intBytes := Base64Dec(strBase64, oBin)

; utiliser code de Hellbent
pBitman := B64ToPBitmap(strBase64)
...
; BinImgToBITMAP(intBytes, pBitmap, oBin) ; not working
; HBITMAP := Gdip_CreateHBITMAPFromBitmap( pBitmap )
return


B64ToPBitmap( Input ){ ; from Hellbent
...
; Found at https://www.autohotkey.com/boards/viewtopic.php?t=59912 and https://www.autohotkey.com/boards/viewtopic.php?t=44115
...
strBase64 := ""
strDestFilePath := A_ScriptDir . "\Clipboard.clip"

; FileAppend, %ClipboardAll%, %strDestFilePath%

FileGetSize, intBytes, %strDestFilePath%
FileRead, oClipboardAllBin, *c %strDestFilePath%
strBase64 := Base64Enc(oClipboardAllBin, intBytes, 64, 0)

Clipboard := strBase64
###_V("", SubStr(strBase64, 1, 200))

strDestFilePath := A_ScriptDir . "\Clipboard-Copy.clip"
intBytes := Base64Dec(strBase64, oNewBin)
oFile := FileOpen(strDestFilePath, "w")
oFile.RawWrite(oNewBin, intBytes)
oFile.Close()

FileRead, Clipboard, *c %strDestFilePath%
...
strDestFilePathNameNoExt := A_ScriptDir . "\Clipboard"
strFormat := "png" ; bmp; jpeg; gif; tiff; png
strDestFilePath := strDestFilePathNameNoExt . "." . strFormat
WinClip.SaveBitmap(strDestFilePath, strFormat)
FileGetSize, intBytes, %strDestFilePath%
FileRead, oImage, *c %strDestFilePath%
strBase64 := Base64Enc(oImage, intBytes, 64, 0)
...
; voir https://www.autohotkey.com/boards/viewtopic.php?p=362293#p362293
; intBytes := Base64Dec(strBase64, oBin)
; BinImgToBITMAP(intBytes, HBITMAP, oBin)
; WinClip.SetBitmap(HBITMAP) ; ##### ne fonctionne pas - comment lire un fichier image en bitmap?

strDestFilePath := strDestFilePathNameNoExt . "-COPY." . strFormat
intBytes := Base64Dec(strBase64, oNewBin)
oFile := FileOpen(strDestFilePath, "w")
oFile.RawWrite(oNewBin, intBytes)
oFile.Close()
