...
strDBFileName := A_WorkingDir . "\..\..\QuickClipboardEditor\QCE-DB3.db"

o_Db := new SQLiteDB
o_Db.OpenDB(strDBFileName)
strDbSQL := "SELECT ClipID, ClipText, Thumbnail FROM History ORDER BY ClipID DESC LIMIT 40;"
...
ILC_MASK := 0x00000001, ILC_COLOR := 0, ILC_COLOR4 := 0x4, ILC_COLOR8 := 0x8, ILC_COLOR16 := 0x10, ILC_COLOR24 := 0x18, ILC_COLOR32 := 0x20, ILC_ORIGINALSIZE := 0x00010000

intIlW := 48
intIlH := 48
hIl := ImageList_Create(intIlW, intIlH, ILC_MASK|ILC_COLOR32|ILC_ORIGINALSIZE, 10, 10)
LV_SetImageList(hIl, 1) ; list mode (default)

/*
oRecordSet.Next(oRow) ; assuming first record contains a thumbnail
pAddr := oRow[3].GetAddress("Blob")
intSize := oRow[3].Size
pBitmap := Gdip_CreateBitmapFromData(pAddr, intSize)
hBitmap := GetPictureHBitmap(pBitmap, intIlW, intIlH, intActualW, intActualH)
*/
pBitmap := Gdip_CreateBitmapFromFile("C:\Dropbox\AutoHotkey\SandBox\SQLite_BLOB\clipboard-48.bmp")
hBitmap := GetPictureHBitmap(pBitmap, intIlW, intIlH, intActualW, intActualH)
...
Loop
{
	...
	LV_Add("Icon" . intIndex + 1, "", "intIndex=" . intIndex . " / A_Index=" . A_Index . " (" . oRow[1] . ") Text: " . oRow[2])
}
oRecordSet.Free()
/*
Loop, 2
